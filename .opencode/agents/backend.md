...
description: >-
  ...
  feature o cambio grande de API: este agente revisa el diff, lo ajusta a las convenciones
  (multi-tenant, RBAC, MongoDecimal, alta en router.py) y lo verifica con ruff/mypy/uvicorn.
  Invócalo también DIRECTAMENTE para cambios pequeños: un endpoint más, un campo en un
...
- **Consulta primero el grafo de graphify.** Si existe `graphify-out/graph.json`, arranca con
  ...
  dependencias FastAPI implicados y sus relaciones. Lee a fondo **solo** los archivos que el
  grafo señale. `Grep`/`Glob` a mano solo si el grafo no cubre lo que buscas o no existe. Ahorra
...
- Tarea de feature grande **sin** implementación previa: dilo y recomienda que el hilo principal
  la genere primero. Procede directo solo si el cambio es pequeño.
